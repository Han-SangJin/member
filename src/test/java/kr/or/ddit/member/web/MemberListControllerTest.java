...
public class MemberListControllerTest extends WebTestConfig {
	...
	public void processSuccessTest() throws Exception {
		...
		.andExpect(model().attributeExists("pages"))		// memberList 이름으로 된 키값이 있는지 확인
		.andExpect(model().attributeExists("page"))		// memberList 이름으로 된 키값이 있는지 확인
		.andExpect(model().attributeExists("pageSize"));		// memberList 이름으로 된 키값이 있는지 확인
	}
