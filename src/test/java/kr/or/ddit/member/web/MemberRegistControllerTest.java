...
//		FileInputStream fis = new FileInputStream("D:\\A_TeachingMaterial\\6.JspSpring\\workspace\\spring\\src\\test\\resources\\kr\\or\\ddit\\upload\\sally.png");
		...
		.andExpect(view().name("redirect:/member/view?userid=id테스트2"));	// 200으로 바꿔서 실행 해도 insert 값이 0 나와서 실패 조건문으로 가기때문에 페이지 에러발생
	...
	public void processFailTest() throws Exception {
		...
		mockMvc.perform(fileUpload("/memberRegist/process")
						...
						.param("addr2", "테스트")
						.param("zipcode", "1"))
		.andExpect(status().is3xxRedirection())						//200 은 정상
		.andExpect(view().name("redirect:/member/view?userid=noylit"));
	}
